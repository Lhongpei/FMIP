...
class MILPGCN(nn.Module):
    def __init__(
        ...
        super(MILPGCN, self).__init__()
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        time_embed_dim = hidden_dim//2
        self.only_discrete = only_discrete
        # Embedding layers
        # self.var_embedding = FourierEncoder(level=math.ceil(hidden_dim/(2*input_dim_x_var)))
        # self.cons_embedding = FourierEncoder(level=math.ceil(hidden_dim/(2*input_dim_x_cons)))
        # var_enc_dim = math.ceil(self.hidden_dim/(2*input_dim_x_var))*2*input_dim_x_var
        # cons_enc_dim = math.ceil(self.hidden_dim/(2*input_dim_x_cons))*2*input_dim_x_cons
        # self.x_var_linear = torch.nn.Linear(var_enc_dim, self.hidden_dim)
        # self.x_cons_linear = torch.nn.Linear(cons_enc_dim, self.hidden_dim)
        ...
        # Convolution layers
        self.conv_vars2cons = nn.ModuleList([TripartiteGCNConv(hidden_dim) for _ in range(num_layers)]) \
            if not self.only_discrete else \
            nn.ModuleList([BipartiteGCNConv(hidden_dim) for _ in range(num_layers)])
        self.conv_cons2Ivars = nn.ModuleList([BipartiteGCNConv(hidden_dim) for _ in range(num_layers)])

        if not self.only_discrete:
            self.conv_cons2Cvars = nn.ModuleList([BipartiteGCNConv(hidden_dim) for _ in range(num_layers)])
    ...
    # def _broadcast_dim(self, x_var_shape, x_cons_shape, for_primal, for_dual):
    #     if not (for_primal or for_dual):
    #         raise ValueError("Either for_primal or for_dual must be True.")
    #     elif for_primal and for_dual:
    #         broadcast_dim = x_var_shape[0] + x_cons_shape[0]
    #     elif for_primal:
    #         broadcast_dim = x_var_shape[0]
    #     else:
    #         broadcast_dim = x_cons_shape[0]
    ...
    def _forward_multimodal(
        ...
        for conv_vars2cons,  \
            ...
            x_Cvars = x_Cvars_in + time_emb_tmp[broadcast_dim_Ivars: ] + out_layer_Cvars(x_Cvars)
